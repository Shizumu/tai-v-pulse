  ...
  return (
    <main className="app-shell legal-shell">
      ...
      <section className="panel legal-document">
        <div><p className="section-kicker">ABOUT</p><h2>工具定位</h2><p>台V Pulse 是第三方製作、在使用者電腦本機執行的資料整理工具，並非 YouTube 或 Google 的官方產品，也未獲其背書。使用者須自行申請及保管 YouTube Data API Key；若使用私人 Analytics 直接連線，也須使用自己的 Google OAuth 桌面應用程式。</p></div>
        <div><p className="section-kicker">DATA SOURCES</p><h2>資料來源與保存</h2><p>公開頻道、影片及直播資料來自 YouTube Data API。這類未經頻道主 OAuth 授權的公開 API 快照最多保存 30 天，期限到達後會由本機排程刪除。經使用者明確授權的頻道與私人 Analytics 來自 YouTube Data API 及 YouTube Analytics API，只使用帳戶與 Analytics 唯讀權限；OAuth 同步、YouTube Studio 匯入與手動補充資料可依設定保留一個月至十年或永久。</p><p>資料、API Key、OAuth 設定與 token 預設只保存在使用者自己的電腦，不會由本專案作者集中收集。中斷 OAuth 連線會嘗試向 Google 撤銷授權並刪除本機 token 與直接同步資料；也可以停止程式後刪除本機資料庫及 `work/oauth/`，清除全部內容。</p></div>
        ...
        <div><p className="section-kicker">PRIVACY & SECURITY</p><h2>隱私與安全責任</h2><p>請勿匯入與分析無關的敏感資料，也不要提交 `.env`、API Key、OAuth JSON／token、`work/`、本機資料庫或 YouTube Studio 匯出檔。OAuth 設定與 token 會以目前 Windows 使用者的 DPAPI 加密，但已登入的電腦帳號仍可能存取程式與私人資料；長期保存前應確認電腦帳號、磁碟與備份位置受到妥善保護。</p></div>
      ...
      <LegalFooter context="使用、隱私與授權聲明" note="最後更新：2026-07-29" />
    </main>
  );
}
