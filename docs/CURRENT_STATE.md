...
- 既有 `0.8.3`、`0.8.4` 與 `outputs/pending-version-20260729-oauth-help-final/` 內候選包是歷史產物；正式分享應使用 `outputs/tai-v-pulse-0.9.0-setup.exe` 及同名 `.sha256`。
...
- 尚未在另一台乾淨 Windows 電腦完成 `0.9.0` 正式包的首次安裝、執行環境自動安裝、啟動、長時間運作及資料保留實機驗收。
- `0.9.0` Windows 安裝程式尚未進行 Authenticode 程式碼簽章，SmartScreen 可能顯示「未知發行者」；SHA-256 只能驗證檔案一致，不能取代簽章。
...
- 對外分享時只提供 `0.9.0` 安裝程式與同名 SHA-256；`0.8.4` 及更早版本保留為歷史產物，不再宣稱包含本次功能。
- 由使用者在 Google Cloud 建立自己的桌面 OAuth 用戶端，以實際頻道依序驗證首次授權、立即同步、24 小時續期、Google 撤銷與重新連結；驗收時不要分享 JSON 或 token。
- 在乾淨 Windows 電腦驗證 `0.9.0` 正式包的首次安裝、缺少執行環境時的引導、啟動、升級及資料保留。
