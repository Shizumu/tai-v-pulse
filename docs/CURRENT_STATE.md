...
- 尚未在另一台乾淨 Windows 電腦完成 `0.11.0` 正式包的首次安裝、執行環境自動安裝、啟動、長時間運作及資料保留實機驗收，也尚未實機完成 0.10.x → 0.11.0 的內建更新流程。
- `0.11.0` Windows 安裝程式仍未進行 Authenticode 程式碼簽章，SmartScreen 可能顯示「未知發行者」；SHA-256 只能驗證檔案一致，不能取代簽章。
